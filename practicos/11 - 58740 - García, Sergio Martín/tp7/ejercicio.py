...
def graficar_resultados(x, y, x_pred, y_pred):
    ...
    plt.plot(x_pred, y_pred, label='Curva de Ajuste', color='red')
...
learning_rate = st.sidebar.slider('Aprendizaje', 0.0001, 0.1, 0.01)
epochs = st.sidebar.slider('Repeticiones', 10, 1000, 100)
hidden_size = st.sidebar.slider('Neuronas Capa Oculta', 1, 100, 10)
...
if st.sidebar.button('Entrenar'):
    ...
    st.metric(label="Error Final", value=f"{loss_history[-1]:.4f}")
    plt.figure()
    plt.plot(loss_history, color='green', label='Pérdidas')
    plt.xlabel('Época')
    plt.ylabel('Pérdida')
    plt.legend()
